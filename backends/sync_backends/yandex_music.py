...
class SyncYandexMusicBackend(SyncAbstractBackend):
    ...
    def _get_file(self, track: Track) -> Dict:
        title = track.title
        artist = (
        ...
        self._add_id3_tags(track=track)
        return dict(file_path=self.file_path, title=title)

    def _add_id3_tags(self, track: Track):
        try:
    ...
    def get(self) -> YandexMusicBackendResult:
        track: Track = self._find_object()
        downloaded: Dict = self._get_file(track=track)
